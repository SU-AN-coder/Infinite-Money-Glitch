...
export class AIDevProofGenerator {
  ...
  constructor(projectRoot: string = '.', outputPath: string = './ai-dev-proof.json') {
   ...
   */
  async generate(): Promise<AIDevProof> {
   ...
   * Save proof to file
   */
  async save(proof: AIDevProof): Promise<string> {
   ...
   */
  async generateAttestation(proof: AIDevProof): Promise<string> {
    const lines = [
    ...
    lines.push(
      ...
      '',
      `**Document Checksum (SHA-256):** \`${proof.checksum}\``,
      '',
  ...
  private async collectSourceFingerprints(): Promise<FileFingerprint[]> {
  ...
  private getGitCommits(): GitCommit[] {
    try {
      const output = execSync(
        'git log --format="%H|%an|%aI|%s" -n 100',
  ...
  private calculateTimeline(commits: GitCommit[]): DevelopmentTimeline {
    if (commits.length === 0) {
      return {
        firstCommit: 'N/A',
        lastCommit: 'N/A',
  ...
  private computeChecksum(proof: Omit<AIDevProof, 'checksum'>): string {
